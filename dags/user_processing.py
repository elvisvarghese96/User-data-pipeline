...
import json
from pandas import json_normalize # type: ignore # used in _process_user
#from airflow import Database # type: ignore


def _process_user(ti):
    user = ti.xcom_pull(task_ids="extract_user_data")
    if user is None:
        raise AirflowException("No user data found from extract_user_data task!")
    user = user['results'][0]
